...
export default function PYQList() {
  ...
  const userClass = pathSegments.includes('10') ? '10' : '12';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch PYQ data
        const response = await axios.get(`${API_URL}/free/year-wise-questions/${userClass}/2024`, {
          headers: {
            "Content-Type": "application/json",
          },
        });

        const pyqData = response.data.data.questions.subjects.flatMap(subject => subject.questions);

        // Map data and include status based on papersAttempted
        const mappedData = pyqData.map((question) => ({
          id: question.qid,
          title: question.title,
          difficulty: question.difficulty.charAt(0).toUpperCase() + question.difficulty.slice(1),
          hasTopperSolution: Boolean(question.answer),
          hasVideoSolution: Boolean(question.question),
          videoUrl: question.question || null,
          tag: question.year === 2024 ? "Free" : ["2017", "2016", "2015"].includes(question.year) ? "Coming Soon" : "Subscription Required",
        }));
